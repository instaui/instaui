 ...
 */
export const RelationField = ({ field, apiClient, rules, isDisabled, form, }) => {
    ...
    const debouncedFetchOptions = useCallback(debounce((value) => {
    ...
    return (_jsx(Form.Item, { name: field.key, label: field.label, rules: rules, children: _jsx(Select, { showSearch: true, placeholder: field.placeHolder || `Select ${field.label}`, disabled: isDisabled, loading: loading, options: options, value: currentValue, searchValue: searchValue, filterOption: (input, option) => option?.label?.toString().toLowerCase().includes(input.toLowerCase()) ?? false, onSearch: handleSearch, onPopupScroll: handlePopupScroll, onChange: (value) => form.setFieldValue(field.key, value), onDropdownVisibleChange: handleDropdownVisibleChange, notFoundContent: loading ? _jsx(Spin, { size: "small" }) : null, allowClear: true, defaultActiveFirstOption: false, autoClearSearchValue: false, popupMatchSelectWidth: false, listHeight: 256, getPopupContainer: (triggerNode) => triggerNode.parentNode }) }));
};
